...
module.exports = (provider, resumeRouteName) => {
  ...
  return async function interactions(ctx, next) {
    ...
    if (interaction) {
      ...
      console.log({
        message: 'Set cookies...',
        issuer: instance(provider).issuer,
        mountPath: instance(provider).mountPath,
        returnTo,
        resumeRouteName,
        destination,
        interactionPath: url.parse(destination).pathname,
        resumePath: url.parse(returnTo).pathname,
        cookies: ctx.cookies,
        interactionCookie: ctx.cookies.get(provider.cookieName('interaction')),
        resumeCookie: ctx.cookies.get(provider.cookieName('resume')),
      })
